...
class Levels {
    ...
    fun loadEtoileData(context: Context): List<Niveau> {
        val fileName = "niveau.json"
        ...
        val gson = Gson()
        val type = object : TypeToken<List<Niveau>>() {}.type
    ...
    fun saveEtoileData(context: Context, niveau: List<Niveau>) {
        val fileName = "niveau.json"
        val internalFile = File(context.filesDir, fileName)
        val gson = Gson()
        val jsonString = gson.toJson(niveau)
