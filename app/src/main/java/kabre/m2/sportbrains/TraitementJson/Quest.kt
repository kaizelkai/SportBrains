...
class Quest {
    ...
    fun loadQuestData(context: Context, userScore: Int, totalStars: Int, userDepence: Int): List<QuestModel>? {
        ...
        return try {
            ...
            // Mettre à jour les données de quêtes
            updateQuestData(taches, userScore, totalStars, userDepence)

            // Trier les quêtes mises à jour
            val sortedTaches = taches.sortedWith(
                compareByDescending<QuestModel> { it.progress >= it.max }
                    .thenBy { it.nom }
            )

            // Sauvegarder la liste triée dans le stockage interne
            saveQuestDataToInternalStorage(context, sortedTaches)
